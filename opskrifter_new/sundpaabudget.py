...
def parse_method(soup):
    return [f"{count + 1} {li.text}" for count, li in enumerate(soup.find("ul", {"class": "directions-list"}).findChildren())]

def parse_rating(soup):
    return soup.find("small", {"class": "wpzoom-rating-average"}).text
...
def get_json(url, x):
    ...
    ingredients = [
        parse_ingredients(soup, x)
        for x in range(
            len(get_ingredient(soup, 'wpzoom-rcb-ingredient-amount'))
        )
    ]

    print(f"Ingre: {ingredients}")
...
def main():
    ...
    with open("opskrifter_new/recipes.json", "w", encoding="utf8") as file:
