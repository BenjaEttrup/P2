  ...
  render() {
    return (
      <div className="SearchStashProduct" >
        <div className="searchWidth" onBlur={this.handleBlur} onFocus={() => { this.setState({ showDropdown: true }) }
        } >
          <div class="input-group rounded">
            <input type="search" class="form-control rounded" value={this.state.searchValue} placeholder="Search" aria-label="Search" aria-describedby="search-addon"
          ...
          <div class="searchResults" >
            <ul className="list-group" >
              {this.state.showDropdown ? (this.state.products !== undefined ? this.state.products.map((product) => {
                return (
                  <button type="button" className="list-group-item list-group-item-action" onClick={() => { this.addIngredient(product); this.resetSearch() }}>
                    {product.title}
                  </button>
                )
              }) : "") : ""
